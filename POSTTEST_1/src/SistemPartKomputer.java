...
void main() {
    ...
    listPart.add(new PartKomputer("Intel 14900KF", 5, 7950000));
    ...
    do {
        ...
        switch (pilihan) {
            ...
            case 7 -> {
                ...
                lap.cetakLaporan(riwayatTransaksi, listPart);
            }
...
void updateHarga() {
    ...
    if (indeks >= 0 && indeks < listPart.size()) {
        ...
        System.out.println("Part terpilih: " + p.nama);
        System.out.print("Masukkan Harga Baru: ");
        p.harga = input.nextDouble();

        System.out.println(">> Berhasil: Harga " + p.nama + " diperbarui.");
    } else {
...
void transaksiPenjualan() {
    ...
    if (indeks >= 0 && indeks < listPart.size()) {
        ...
        if (p.stok >= jumlah) {
            ...
            Transaksi t = new Transaksi(p.nama, jumlah, total);
            riwayatTransaksi.add(t);

            System.out.println("\n>> Transaksi Berhasil!");
            t.cetakStruk();
        } else {
            System.out.println(">> Gagal: Stok tidak mencukupi (Tersisa: " + p.stok + ")");
        }
    } else {
        System.out.println(">> Error: Nomor tidak valid!");
    }
...
void hapusPart() {
    ...
    if (indeks >= 0 && indeks < listPart.size()) {
        ...
        System.out.println(">> Berhasil menghapus: " + p.nama);
    } else {
